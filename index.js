/* empty css                      */import{a as F}from"./assets/vendor-BkCUij8E.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const d of i)if(d.type==="childList")for(const v of d.addedNodes)v.tagName==="LINK"&&v.rel==="modulepreload"&&r(v)}).observe(document,{childList:!0,subtree:!0});function a(i){const d={};return i.integrity&&(d.integrity=i.integrity),i.referrerPolicy&&(d.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?d.credentials="include":i.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function r(i){if(i.ep)return;i.ep=!0;const d=a(i);fetch(i.href,d)}})();const R="https://tasty-treats-backend.p.goit.global/api",P=document.getElementById("category-list"),G=document.querySelector(".all-categories-button"),K=document.getElementById("search-input"),T=document.getElementById("recipe-grid"),V=document.getElementById("empty-recipe-grid"),gt=document.getElementById("reset-button"),_=document.getElementById("time-select"),B=document.getElementById("area-select"),A=document.getElementById("ingredient-select"),L=document.getElementById("pagination");let x=null,j="",H="",q="",N="",h=1;const pt=9;let k=[];function Z(o){return k.some(s=>s._id===o)}function vt(){localStorage.setItem("favorites",JSON.stringify(k))}function yt(){const o=localStorage.getItem("favorites");if(o)try{JSON.parse(o).forEach(a=>k.push(a))}catch{localStorage.removeItem("favorites")}}async function ht(){try{const s=(await F.get(`${R}/categories`)).data;P.innerHTML="",s.forEach(a=>{const r=document.createElement("li"),i=document.createElement("button");i.textContent=a.name,i.classList.add("category-btn"),i.addEventListener("click",()=>{x=a.name,h=1,D(),E()}),r.appendChild(i),P.appendChild(r)})}catch(o){console.error("Kategori yüklenirken hata:",o)}}function D(){G.classList.remove("active"),P.querySelectorAll("li").forEach(o=>o.classList.remove("active")),x===null?G.classList.add("active"):P.querySelectorAll("li").forEach(o=>{o.querySelector("button").textContent===x&&o.classList.add("active")})}G.addEventListener("click",()=>{x=null,h=1,D(),E()});K.addEventListener("input",()=>{j=K.value.trim(),h=1,E()});gt.addEventListener("click",()=>{x=null,j="",H="",q="",N="",K.value="",_.value="",B.value="",A.value="",h=1,D(),E()});_.addEventListener("change",()=>{H=_.value,h=1,E()});B.addEventListener("change",()=>{q=B.value,h=1,E()});A.addEventListener("change",()=>{N=A.value,h=1,E()});function bt(){const o=[15,30,45,60,90,120];_.innerHTML='<option value="">All times</option>',o.forEach(s=>{const a=document.createElement("option");a.value=s,a.textContent=`${s} minutes`,_.appendChild(a)})}async function wt(){try{const s=(await F.get(`${R}/areas`)).data;B.innerHTML='<option value="">All areas</option>',s.forEach(a=>{const r=document.createElement("option");r.value=a.name,r.textContent=a.name,B.appendChild(r)})}catch(o){console.error("Area options yüklenirken hata:",o)}}async function Lt(){try{const s=(await F.get(`${R}/ingredients`)).data;A.innerHTML='<option value="">All ingredients</option>',s.forEach(a=>{const r=document.createElement("option");r.value=a._id,r.textContent=a.name,A.appendChild(r)})}catch(o){console.error("Ingredient options yüklenirken hata:",o)}}async function E(o=1){h=o,T.innerHTML="Loading...";try{const s={page:h,limit:pt};x&&(s.category=x),j&&(s.title=j),H&&(s.time=H),q&&(s.area=q),N&&(s.ingredient=N);const a=await F.get(`${R}/recipes`,{params:s}),r=a.data.results||[],i=a.data.totalPages||1;if(!r.length){T.innerHTML="",V.innerHTML=`<svg
      ...
      >
        ...
        <p>No recipes found.</p>`,L.innerHTML="";return}V.innerHTML="",xt(r),$t(i,h)}catch(s){console.error("Tarifler yüklenirken hata:",s),T.innerHTML="<p>Error loading recipes.</p>",L.innerHTML=""}}function Et(o){let s="";for(let a=0;a<5;a++)a<parseInt(o)?s+=` <svg
      ...
      >
        ...
        </svg>`}function Q(o){return(o==null?void 0:o._id)||(o==null?void 0:o.id)}function xt(o){T.innerHTML="",o.forEach(s=>{const a=document.createElement("div");a.className="recipe-card";const r=s.thumb||"https://via.placeholder.com/280x180?text=No+Image";a.style.backgroundImage=`linear-gradient(0.94deg, rgba(5, 5, 5, 0.6) 4.82%, rgba(5, 5, 5, 0.5) 18.72%), url(${r})`;const i=parseFloat(s.rating).toFixed(1);a.innerHTML=`
    ...
    `,a.querySelector(".favorite-btn").addEventListener("click",()=>{W(s)}),a.querySelector(".see-recipe-btn").addEventListener("click",()=>{const d=Q(s);typeof window.openPopup=="function"?window.openPopup(d||s):console.error("⚠️ openPopup fonksiyonu tanımlı değil!")}),T.appendChild(a)})}function W(o,s=!1){Z(o._id)?k=k.filter(r=>r._id!==o._id):k.push(o);const a=document.getElementById(`favorite-button-${o._id}`);a&&(a.innerHTML=Y(o._id)),!s&&vt()}function $t(o,s){L.innerHTML="";const a=(r,i,d=!1,v=!1,C="")=>{const y=document.createElement("button");return y.innerHTML=r,y.className=C,v&&y.classList.add("active"),d&&y.classList.add("disabled"),y.disabled=d,y.addEventListener("click",()=>{d||E(i)}),y};L.appendChild(a(`<svg
...
`).map(n=>n.trim()).filter(Boolean).map(n=>({name:n})):Array.isArray(t==null?void 0:t.components)?t.components:Array.isArray(t==null?void 0:t.extendedIngredients)?t.extendedIngredients:[];function at(t=""){const n=String(t).trim();if(!n)return"";if(/^[a-zA-Z0-9_-]{11}$/.test(n))return n;const e=n.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/)||n.match(/[?&]v=([a-zA-Z0-9_-]{11})/);return e?e[1]:""}function rt(t){const n=(t==null?void 0:t.youtube)||(t==null?void 0:t.video)||(t==null?void 0:t.youtubeUrl)||(t==null?void 0:t.youtubeLink)||(t==null?void 0:t.yt)||"",e=at(n);return e?`https://www.youtube.com/embed/${e}?rel=0&modestbranding=1`:""}window.__pmEscHandler=window.__pmEscHandler||null,window.openPopup=async function(t){const n=document.getElementById("home-filter-popup-overlay"),e=document.getElementById("home-filter-popup-content");if(!n||!e)return console.error("❌ #popup-overlay/#popup-content yok.");n.setAttribute("role","dialog"),n.setAttribute("aria-modal","true");let c=typeof t=="object"?t:a().find(l=>$(l)===String(t))||{_id:t};lt(e,c),n.classList.remove("hidden"),document.body.style.overflow="hidden";try{const l=$(c),[f,m]=await Promise.all([l?y(l):Promise.resolve(null),C()]);c={...c,...f||{}},ct(e,n,c,m)}catch(l){console.error("Popup fetch error:",l),e.innerHTML=dt(l),z(n)}},window.closePopup=function(){const t=document.getElementById("home-filter-popup-overlay");t&&(t.classList.add("hidden"),document.body.style.overflow="",window.__pmEscHandler&&(document.removeEventListener("keydown",window.__pmEscHandler),window.__pmEscHandler=null))};function it(t){const n=rt(t);return n?`
      ...
      </div>`,z(document.getElementById("home-filter-popup-overlay"))}function ct(t,n,e,c){var O;const l=((O=e==null?void 0:e.time)==null?void 0:O.minutes)??(e==null?void 0:e.time)??"",f=((e==null?void 0:e.tags)||[]).map(p=>`<span class="tag-chip">#${b(p)}</span>`).join(""),g=ot(e).map(p=>{const w=nt(p,c),S=st(p);return`<div class="ing-row"><span class="ing-name">${b(w)}</span><span class="ing-measure">${b(S)}</span></div>`}).join(""),M=i($(e))?"Remove from favorite":"Add to favorite";t.innerHTML=`
      <button class="popup-x" type="button" aria-label="Close">×</button>
      ...
      <div class="ingredients">
        ${g||'<div class="ing-row"><span class="ing-name">Ingredients</span><span class="ing-measure">N/A</span></div>'}
      </div>
    ...
    `,z(n),t.querySelector("#pm-fav-btn").addEventListener("click",()=>{const p=a(),w=$(e);W(e,!0),i(w)?(r(p.filter(S=>$(S)!==w)),t.querySelector("#pm-fav-btn").textContent="Add to favorite"):(r([{...e},...p]),t.querySelector("#pm-fav-btn").textContent="Remove from favorite")}),t.querySelector("#pm-rate-btn").addEventListener("click",()=>{mt(e)})}function dt(t){return`
      ...
      </div>`}function z(t){const n=document.querySelector(".popup-x");n&&(n.onclick=()=>window.closePopup()),t.onclick=e=>{e.target===t&&window.closePopup()},window.__pmEscHandler&&document.removeEventListener("keydown",window.__pmEscHandler),window.__pmEscHandler=e=>{e.key==="Escape"&&window.closePopup()},document.addEventListener("keydown",window.__pmEscHandler)}async function ut(t,n,e){const c=$(t);if(!c)throw new Error("Missing recipe id");const l=`${o}/recipes/${c}/rating`,f={rate:Number(n)};e&&(f.email=e);const m=await fetch(l,{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify(f),mode:"cors",cache:"no-store"});let g="";try{g=await m.clone().text()}catch{}if(console.groupCollapsed("[rating] response"),console.log("URL:",l),console.log("Status:",m.status,m.statusText),console.log("Headers:",Object.fromEntries(m.headers.entries())),console.log("Body:",g),console.groupEnd(),!m.ok)try{const M=JSON.parse(g||"{}");throw new Error(M.message||`${m.status} ${m.statusText}`)}catch{throw new Error(g||`${m.status} ${m.statusText}`)}try{return JSON.parse(g||"{}")}catch{return{}}}function mt(t){const n=document.getElementById("home-filter-popup-content");if(!n)return;const e=n.querySelector(".rating-overlay");e&&e.remove();const c=document.createElement("div");c.className="rating-overlay",c.innerHTML=`
    ...
    `,n.appendChild(c);const l=c.querySelector(".rating-card"),f=l.querySelector(".rating-x"),m=l.querySelector(".rating-email"),g=l.querySelector(".rating-send"),M=l.querySelector(".rating-stars"),O=l.querySelector(".rating-value"),p=l.querySelector(".rating-hint");let w=0;const S=(u="")=>/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(u),ft=u=>{w=u,O.textContent=u.toFixed(1),l.querySelectorAll(".star-container").forEach(I=>{const J=Number(I.dataset.star),U=I.querySelector("svg path");u>=J?U.setAttribute("fill","#FFC107"):u>=J-.5?U.setAttribute("fill",`url(#halfGradient-${J})`):U.setAttribute("fill","#D6D6D6")}),g.disabled=!(w&&S(m.value))};M.addEventListener("click",u=>{const I=u.target.closest(".half-btn");I&&ft(Number(I.dataset.val))}),m.addEventListener("input",()=>{g.disabled=!(w&&S(m.value))}),f.onclick=()=>c.remove(),c.addEventListener("click",u=>{u.target===c&&c.remove()}),g.addEventListener("click",async()=>{p.textContent="",g.disabled=!0;try{await ut(t,w,m.value.trim()),p.textContent="Thanks for your rating!",p.classList.add("ok"),setTimeout(()=>c.remove(),900)}catch(u){p.textContent=String((u==null?void 0:u.message)||"Failed to send rating. Please try again."),p.classList.remove("ok"),g.disabled=!1}})}})();
